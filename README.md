...
| 분리된 소스 | SDR (dB) | 분석 |
| :--- | :--- | :--- |
| **Drums** | **+2.18 dB** | 악기의 가장 뚜렷한 주파수 특성으로 가장 잘 분리됨. |
| **Vocals** | **+1.30 dB** | 사람의 목소리와 악기를 어느 정도 구분하는 데 성공. |
| **Bass** | **+0.88 dB** | 초기 음수에서 데이터 증량 후 양수로 전환. 저음역대 구분 능력 확보 중. |
| **Other** | **-0.29 dB** | 0dB에 근접. 여전히 음수이나 초기(-7dB) 대비 소스의 형체를 알아볼 수 있는 수준. |

### 3. SOTA 모델 자원 효율성 검증 (실패 분석)
U-Net 토이 모델의 한계를 확인한 후 SOTA 모델 학습을 시도했으나, 다음과 같은 자원 한계에 봉착했습니다.

| 모델 | 시도 결과 | 분석 |
| :--- | :--- | :--- |
| **Demucs v4** | 학습 시작 실패 | **VRAM 한계**로 인한 OOM 발생 |
| **Demucs v2** | 학습 중단 | 1 Epoch 학습 중 **Colab 런타임(12시간) 초과** |
| **Conv-TasNet** | 비효율성 확인 | 유의미한 성능(100 epoch) 위해 수백~수천 시간 소요 예상 |
